...
sns.countplot(combined_data['Electrical'], data=combined_data) #To get the most frequent value which is Mode
...
#Create new features
str_columns = combined_data.select_dtypes(include=['number']).columns.values
num_columns = combined_data.select_dtypes(exclude=['number']).columns.values
