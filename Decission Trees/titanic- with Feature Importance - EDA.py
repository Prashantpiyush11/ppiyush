...
paramDT = model_selection.GridSearchCV(tree_estimator, dt_grid, cv=10) #Evolution of tee
paramDT.fit(X_train, y_train) #Building the tree
print(paramDT.best_score_) #Best score
print(paramDT.best_params_)
print(paramDT.score(X_train, y_train)) #Train score  #Evalution of tree
...
importances = paramDT.best_estimator_.feature_importances_
...
fe_df = pd.DataFrame({'feature':features, 'importance': importances}) #You may notice that feature	importance "Title_Mr" has more importance
...
X_test.shape
X_test.info()
titanic_test['Survived'] = paramDT.predict(X_test)
